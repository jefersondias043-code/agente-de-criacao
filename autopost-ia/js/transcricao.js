 ...
 *
 * CÓPIA DECLARADA. Este arquivo é idêntico a `src/transcricao.js`, e há teste
 * que quebra se os dois divergirem. O AutoPost é um pacote fechado — ele tem
 * de funcionar aberto direto, por `file://`, sem a plataforma-mãe em volta —,
 * então não dá para importar de `src/`. Duas casas, uma implementação, e um
 * teste no meio para que continue sendo uma só.
 *
 * O `callLLM` daqui tem OUTRA assinatura (sys, user, json, maxTokens). Por isso
 * quem chama daqui passa `call` sempre: o atalho de usar o global só serve na
 * plataforma-mãe.
 * ========================================================================== */
